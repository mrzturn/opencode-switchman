...
## 四、水位（插件实测硬执行，勿自估）
本会话上下文由插件实测并每轮注入 \`[水位·会话]\` 行；超线后读取类工具（read/glob/grep/bash）会被先提醒后硬拦（deny 附 economy 改派建议），不要试图绕行。规则：{{SOFT}} 起扫描/读取一律委派 economy；{{HARD}} 起停新读取只收尾交付（git/测试/lint 验证类命令仍可跑）；{{FORCE}} 必须立即压缩（/handover 或摘要归档拆新会话）。阈值 jsonc \`context.*\` 可调。
