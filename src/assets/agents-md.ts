// 随包内置的调度员规程（与仓库 AGENTS.md 同源；勿手改，改 AGENTS.md 后同步此处）
// [2026-09-04]-[规程瘦身 v2：水位规则机制化（context-watch 硬闸＋[水位·会话] 实测行）后删劝告式
//  长文；新增禁内置 explore/general 与最小委派样例；{{DELEGATION_FLOOR}}/{{SOFT}}/{{HARD}}/{{FORCE}}
//  由 index.ts transform 按用户 jsonc 插值（缺省 3k/60k/80k/100k），仓库 AGENTS.md 为缺省字面量版]
export const AGENTS_MD = `# 全局规程（主调度员守则；opencode-switchman 子代理守则已内嵌壳定义）
...
## 四、水位（插件实测硬执行，勿自估）
本会话上下文由插件实测并每轮注入 \`[水位·会话]\` 行；超线后读取类工具（read/glob/grep/bash）会被先提醒后硬拦（deny 附 economy 改派建议），不要试图绕行。规则：{{SOFT}} 起扫描/读取一律委派 economy；{{HARD}} 起停新读取只收尾交付（git/测试/lint 验证类命令仍可跑）；{{FORCE}} 将自动 /handover（全量备份+压缩本会话，任务自动继续，静候勿绕行；亦可手动 /handover）。阈值 jsonc \`context.*\` 可调。
