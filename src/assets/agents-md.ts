...
//  3k/50k/90k/130k since the 2026-09-15 watermark retune); {{WORKSPACE_DIR}} = the per-session artifact workspace relative path (or a neutralized section
...
// [2026-09-18]-[v1.1 anti-fragmentation: §5 budget sentence gains the one-shot headroom grant (reads up to the remaining
//  per-turn headroom land in one bounded read) and the continuation-completion rule — matches src/context-watch.ts]
...
4. Subagent hard cap: a shell subagent whose context reaches the cap is force-terminated — its final output is a detailed progress summary returned to you as the task result, and its session can never be resumed (task_id resume denials are permanent). On such a denial, start a fresh dispatch with a new self-contained prompt re-seeding only the needed context; never retry the dead session.
5. Shell handoff relay: a shell nearing its context cap is warned by the plugin (conserve → hand off) and instructed to end with \`HANDOFF: inline · progress: n/m · next: <one sentence>\`. Treat that final handover block (completed / key evidence / remaining / next steps) as the authoritative remaining-work state: start a FRESH dispatch whose prompt re-seeds only what the next unit needs (never resume the spent session — its cap termination is permanent, task_id resumes are denied), or, when progress is m/m, fold the results into your own report. Never re-dispatch the same long context back into a fresh shell.
...
Each self-read is costed against a per-call injection budget (default ~1500 tokens; the live cap rides the \`[WATERMARK:SESSION]\` banner line). Reads up to the remaining per-turn headroom (~2× the per-call budget) are granted one-shot — request the exact range you need in a single bounded read instead of fragmenting; reads over the headroom are auto-bounded or rejected with exact bounded-retry params (\`read <file> limit=N offset=M\`), and a follow-up read resuming where the granted window ended completes the original range in one shot. Once the per-turn self-read cap is spent, delegate the turn's remaining reads to an economy shell. Watermarks are lifecycle advice only: past {{SOFT}} prefer delegating new scans; past {{HARD}} wrap up (verification/delivery bash stays open; state-changing git is delivery — run it yourself, never delegate it); at {{FORCE}} compact immediately. Unbounded history dumps (e.g. \`git log -p\` without \`-n\`) are rejected at any context size — scope them or delegate.
