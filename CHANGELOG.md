...
- **Setup hard gate + guided wizard (`/switchman-setup` / `/switchman-setup-chat`)** — unconfigured task pools no longer fall back to "all models": task dispatch is denied in main sessions until every one of the six task pools (`economy`/`mechanical`/`main`/`hard`/`vision`/`review`) has at least one selected model **and** the capability ranking has at least one entry. The guided wizard covers the whole matrix in one pass: **TUI (`/switchman-setup`)** — a resumable intro (Start jumps to the first unfinished step), one multi-select dialog per pool in lane order (Enter toggles, capability tier per row, select-all/clear-all/back; the per-pool confirm persists immediately, minimum one model), a sequential strongest-first ranking pick over exactly the union of the selected models (minimum one), and a done screen with the per-pool summary, ranking order, and any providers still flagged restartRequired; **conversational fallback (`/switchman-setup-chat`)** — the same flow in chat (state overview → per-pool multi-select questions → ranking → CLI persist → verify), for when the TUI plugin surface is unavailable. While setup is incomplete, main sessions carry a `[SETUP]` directive each turn and every task denial relays the remedy; dispatched shells and internal sessions are exempt and there is no waiver flag. The gate rides on the language gate — until the project language preference is configured, the language ask stays the single blocking voice. The wizard writes the same hot-reloaded `pool-config.json`/`capability-rank.json` as the manual overrides, so the gate opens the moment the last file is saved — no restart (a restart only registers brand-new providers). New module `src/setup-gate.ts`; sidebar notice and `[LIMITS]` banner segment report the missing state.
- **Render-time i18n (11 locales)** — sidebar chrome, pool/rank/handover dialogs, status-log notices, and config-CLI copy now localize at render time via a new i18n core (`src/i18n.ts`); locale chain: project language → global `ui.lang` → terminal env (`LC_ALL`/`LANG`) → English, with an English/key fallback that never throws. The six-lane sidebar labels localize display-only (runtime `LANE_ORDER`, pool-config keys, and protocol stay English) with CJK-aware column padding; model-facing protocol copy stays byte-stable English so capture anchors (`[ROUTES]`/`[WATERMARK]`/`[LANG]` lines) never drift.

### Changed

- **Real-fail isolation window shortened 30 min → 5 min** — a shell whose probe passes but whose actual delegation fails (in-memory, per-combo isolation) now auto-unlocks after 5 minutes instead of 30: these states are usually transient vendor blips, and a 30-minute head-of-chain lockout starved lanes far longer than the failure warranted. Rate-limit self-heal keeps its dedicated 10-min TTL; endpoint-class config errors keep the 6h TTL; restart still clears everything.
- **Model rank scoped to the task-pool universe with pool-lane priority ordering** — `/modelRank` now ranks exactly the pool-selected universe: rankable rows are scoped to it, an empty universe gates set/add with guidance (the TUI offers a jump to `/poolConfig`), and non-pool entries (with their anchored scores) are auto-pruned on every rank write. The manual rank becomes the absolute ordering key on pool lanes — it beats tier grouping, effort partitions, and family preference — and pool members are exempt from capability-level floors. The `[LIMITS]` banner surfaces the rankable universe.
- **Economy lane effort preference raised to medium→high→low→minimal** — a medium-less flash model (e.g. glm-5.3-flash) now takes its high face as the economy chain head instead of being pinned to low; `off` stays the lane-tail fallback partition.
- **Self-read budget defragmented (tolerance band, headroom grants, continuation merge)** — reads within R* + deny-retry overhead (~400 tok) now pass outright (denying them costs more than the overshoot saved); per-call grants are sized from the remaining turn headroom (max 2× R*) so demonstrated-range reads land one-shot; bounded continuations of the last granted window merge into one completion shot even past the spent turn budget. Protocol §5, config comments, and dispatch-gate tests are synced to the new semantics.

## [1.1.0] - 2026-09-15

### Added

- **Injection face mode `configured` (`injection.mode`)** — a third shell-injection mode that narrows the per-request task-tool description to exactly what can actually dispatch: when a favorites/visible set is configured, runtime dispatch is already gated by activation (configured ∪ session models) ∩ pool selection, so off-configured chain picks are dead context. `configured` skips the six-lane chain picks and injects only the configured models' shells, plus custom-lane and pool-config force-keeps (both stay dispatch-capable). With no favorites/visible set configured it behaves identically to `chain` (fail-open). Startup-level: restart to apply.
- **Shell subagent soft tiers with inline HANDOFF relay (shared absolute thresholds)** — shell subagents previously jumped straight from "no signal" to the hard-cap termination. Two graceful advisories now precede it, reusing the MAIN session's absolute soft/hard thresholds (soft 50k conserve / hard 90k hand-off by default — subagent context is equally token-expensive as main-session context), so there are no user-facing fraction coefficients: a conserve advisory (stop batch reads and long pastes, switch to targeted grep, keep outputs compact) and a hand-off advisory (finish the current unit, then end the final message with a compact handover block followed by the marker line `HANDOFF: inline · progress: n/m · next: <one sentence>`; that final message returns to the delegating session as the task result). The window-clamped effective cap still pulls the tiers down proportionally, so a tier can never exceed the real termination line and small-window shells keep headroom before it. Both report the measured numbers in absolute k, fire at most once per tier per shell session (in-memory dedup, cleared on session deletion; a restart re-arms), and ride the same per-turn system-prompt surface as the session watermark — the hard cap stays the termination backstop, and `subagentCap: false` disables the advisories together with it. The dispatcher side ships in the same release: the bundled protocol §2 gains the shell-handoff-relay rule (treat the handover block as the authoritative remaining-work state; start a fresh re-seeded dispatch, never resume the spent session). Config surface: the tiers follow the session's `softTokens`/`hardTokens`; the shell's termination line follows `forceTokens` by default, overridable via the now-optional `context.subagentForceTokens` (validated 20k..1M; invalid → deleted, falling back to `forceTokens`, fail-open). Restart to apply.
- **Per-turn `[ROUTE]` routing-discipline line** — main sessions now get one `[ROUTE]` line per turn, pushed after `[WATERMARK:SESSION]`/`[TODO]` under the same rules/banner visibility switch (both off = no line, respecting the zero-injection wish). It carries only what the injected lines lack today: the token cost model (hands-on spends and grows this session's context on every later turn; a dispatch spends a fresh shell context and returns a summary) and the declaration discipline, which protocol §6 widens from "before a major action" to "before each substantive action (the enumeration below is the minimum set, not the ceiling)". The line renders from a single pure module, so the text can never drift between surfaces. Restart to apply.
- **Language-preference ask follows the UI locale** — the first-run ask's three question texts are now localized into all 11 candidate tags (en, zh-CN, zh-TW, ja, ko, es, fr, de, it, pt, ru), detected at ask time from the environment (`LC_ALL` → `LANG`, env-style values like `zh_CN.UTF-8` normalized — `zh-HK` maps to `zh-TW` — with an English fallback when nothing matches) and surfaced once per ask activation in the status log (`lang ask locale=…`). Only the question texts localize: the `switchman-lang n/3:` marker prefix, the directive body and the gate's deny copy stay byte-stable English, so the plugin-side answer capture works identically in every locale. Restart to apply.
- **`dispatch: "off"` project opt-out** — a project can now stand down all dispatch governance by setting the top-level `"dispatch": "off"` in `.switchman/settings.json` (the same file the lang config lives in; read per call, so toggling takes effect on the next task call without a restart). Only the exact string `"off"` opts out — anything else (or an absent field) keeps fleet behavior, and a present-but-different value gets a one-time status-log note. When off, task calls are allowed ungoverned: the whole task path stands down (uninjected-shell and built-in-agent denies with their auto-redirects, all six gates including the breaker deny and the ROUTE_META/semantics checks). Deliberately still on: the subagent hard cap and its resume deny, the language gate, read/watermark gates, auto-handover, breaker failure recording, and the banner/watermark/TODO/`[LANG]` lines — the per-turn `[ROUTE]` line is the only prompt surface that hides. A `.switchman/settings.json` that is not valid JSON at all now warns once per process in the status log (`… is not valid JSON — ignored (lang config falls back to the AGENTS.md marker; "dispatch":"off" not applied)`); the "exists but no valid lang block" case stays silent as before. Restart to apply.

### Changed

- **Default context watermarks retuned 60/80/120k → 50/90/130k** — `context.softTokens` drops to 50,000 (the conserve/scan signal fires earlier), `context.hardTokens` rises to 90,000 (more wrap-up room before reads close) and `context.forceTokens` to 130,000 (longer auto-handover runway). Shell subagent advisories and the shared hard cap follow automatically: the conserve/hand-off tiers now sit at the 50k/90k lines and the default termination line is 130k (still clamped to 90% of the model's context window when known). Explicit `context.*` values in user jsonc are untouched. Restart to apply.
- **Read-only shells gain a read-only bash allowlist** — `ro` shells (the review/observe capability faces) previously denied **all** bash (`permission: { edit: "deny", bash: "deny" }`), so a reviewer could not even run `git diff` on the code it was auditing. `edit: "deny"` stays, and `bash` becomes an opencode permission object: a catch-all `"*": "deny"` first, then `allow` entries (opencode semantics: last matching rule wins, wildcard = zero-or-more chars, compound commands judged per segment) for read-only git subcommands with arbitrary arguments (`git status/diff/log/show/blame/rev-parse/ls-files/grep/shortlog/describe/merge-base`, `git stash list`), listing-only forms of the mutable git commands (`git branch`/`git tag`/`git remote` bare and listing flags, `git worktree list`, `git config --get/--list/-l`), and search/inspect utilities (`rg`, `grep`, `ls`, `fd`, `cat`, `head`, `tail`, `wc`, `tree`, `stat`, `file`, `du`, `df`, `pwd`, `which`, `sort`, `uniq`, `basename`, `dirname`, `realpath`, `date`, `whoami`). `find`/`sed`/`awk`/`echo` stay disallowed (they carry mutation forms: `-delete`/`-exec`, `-i`, `>` redirection); a parsed `git diff … > out` redirect may still pass its pattern — accepted, since `edit: "deny"` covers the edit/write/patch tools; compound commands are judged per segment, so `git diff && rm …` falls to the catch-all deny. The ro shell prompt gains a one-line note stating the restriction so review shells report findings instead of attempting writes. Restart to apply.
- **ROUTE_META gate downgraded from deny to synthesize-and-observe** — a delegation prompt with a missing or unparseable ROUTE_META line is no longer hard-denied: the gate synthesizes a lane-default META (lane from the shell's lane, the lane's default role, matching capability/modality, `source: auto`) and lets the dispatch through to the semantic checks with a status-log note. This also removes the three inconsistent corners where the old deny still bit (auto-redirect off, the review lane, synthesis failure) and collapses the prompt-rewriting synthesis retries into the gate itself — redirected dispatches now carry the prompt as-is. Behavior change on upgrade: projects that relied on the missing-META deny as a discipline prod lose it (the `[ROUTE]` line's declaration discipline is the replacement surface); a **present-but-wrong** META (an illegal field value, or a parseable line missing required fields) still denies with the sample attached — only absence is synthesized. Declaring META remains the contract that unlocks cross-family review ranking and `source: user` pinning. Restart to apply.
...
- **Subagent context hard cap (default 100k tokens)** — a shell subagent whose measured context crosses the cap gets every further tool call denied with a wrap-up order; its next text-only answer (a detailed progress summary) becomes the task result returned to the delegating session, and the session is then permanently terminated — any `task_id` resume is denied, and the registry is persisted in `subagent-cap.json` so termination survives restarts. Fresh dispatches without `task_id` are unaffected.
- **Config** — `context.subagentForceTokens` (default 100000, clamped 20k..1M, window-capped at 90%) and `context.subagentCap` (`true`/`false` master switch).
- **Shell rule 6 pre-arms compliance** — stop on the first cap deny and return the summary; dispatcher protocol §2 documents the orchestrator-side contract (never retry a terminated session).
...
- **/poolConfig "Uncheck all" shortcut** — besides "Select all", the per-pool model dialog now offers a one-shot "Uncheck all" (rebuild mode): instead of unchecking models one by one to keep just a few, clear everything in place and check only the ones to keep. While nothing is checked the previous config stays untouched on disk — exiting without any check keeps the pre-clear selection in effect, and the first check after the clear writes the new list.
...
- **Vision/review lanes no longer collapse to "all unavailable" when a whole provider dies** — lane base chains allocated their few candidate seats by capability tier only, so when every top-seated combo was genuinely down (e.g. Copilot answering `402 monthly quota exceeded` across the board and DeepSeek in arrears), healthy lower-tier shells (GLM) had been sliced out of the chain in advance and the lanes showed "all unavailable→terminal failure protocol" until quotas recovered, restarts included. Two layers fix it: matrix-down shells are now filtered out **before** seat allocation (health-aware seats, same combo-down verdict the registry uses to mark a shell disabled; unknown/missing verdicts still fail-open), and `computeLane` backfills an otherwise-empty chain once from candidates dropped only by health reasons (matrix-down / disabled / breaker) via a relaxed-gate ranking pass — structural and pool-level gates (modality, capability, pool-config, exhaustion, retirement) still reject, so illegitimate candidates are never resurrected; if nothing structurally valid remains, the lane keeps its `exhausted` status and the terminal failure protocol.
- **Proven-exhausted pools sink out of the chain heads under the observe-only contract** — the health-aware fix above keyed on per-combo matrix verdicts, so a Copilot shell whose combo probed "strained" (429 noise from a dead pool) or was unprobed still took the chain-head seat by tier while the pool itself was provably dead (live case: the mechanical lane headed `copilot-53codex-high` with Copilot's monthly quota exhausted — `has_quota:false`, remaining −2560 — and GLM healthy). Chain selection now consumes a snapshot-driven `poolUnavailable` resolver (the same `glmExhausted`/`copilotExhausted`/`deepseekExhausted` math, deliberately **not** the policy-gated routing flags): shells of a proven-exhausted pool are excluded from base-chain seats and dropped with `reason=pool-unavailable`, and may only re-enter as the chain **tail** via the backfill pass when nothing healthy exists (health-suspect candidates still rank ahead). This is selection only — the six dispatch gates never see the resolver, so observe-only (routing:false) pools keep their no-hard-denial contract; missing quota snapshots fail open.
