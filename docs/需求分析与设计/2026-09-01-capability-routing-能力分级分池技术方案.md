...
采用“最高可进入池”而非“只能进入同级池”。高能力模型可以回落执行低等级任务；每个已知的非 S 级中，池内动态评分前二模型可向上补一个等级，防止高等级候选断档。

| 任务池 | 最低能力级别 | 强制结构条件 | 典型任务 | 降级规则 |
|---|---|---|---|---|
| `economy` | L1 | 无 | 清点、提取、格式整理、简单检索 | 可使用任意级别 |
...
补位只允许相邻向上一级，且未知 global 兜底模型永不晋级。每个来源等级只取池内综合评分前二名，补位候选始终排在最多四个本级候选之后，形成最多六项候选链；只有本级候选均因下线、熔断、额度、异族或结构门不可用时才实际派发。其余断档仍沿用现有路由失败协议。
